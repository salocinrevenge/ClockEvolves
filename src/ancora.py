...
class Ancora(Poligono):
    def __init__(self, pos: np.ndarray, escala = 1, angulo = 0, massa = 1, space = None, elasticity = 0.3, friction = 1.0, color = None, categoria = 1) -> None:
        ...
        pontos *= escala
        super().__init__(pontos.tolist(), pos = pos, massa = massa, elasticity = elasticity, friction = friction, color = color, space = space, categoria = categoria, meta_info = {"tipo":"ancora"}, escala=escala)
